...
# initial parameters
ALPHA = 0.4   # capital share in manifacturing home appliances
BETA = 0.8   # capital share in manufacturing metal
KBAR = 128  # Total capital supply
LBAR = 128  # Total labor supply
p = 1      # =Pa/Pm  relative price of ag goods

def F(K,L,alpha=ALPHA):
    """Función manufacturación electrodomésticos"""
    return (K**alpha)*(L**(1-alpha))

def G(K,L,beta=BETA):
    """Función manufacturación metales"""
...
def edgeplot(LA, Kbar=KBAR, Lbar=LBAR,alpha=ALPHA,beta=BETA):
    ...
    print("(LE,KE)=({:4.1f}, {:4.1f})  (QE, QM)=({:4.1f}, {:4.1f})  RTS={:4.1f}"
    ...
    ax.set_xlabel(r'$L_E -- Labor$', fontsize=16)
    ax.set_ylabel('$K_E - Capital$', fontsize=16)
    #plt.show()
